...
const FaceModelSelect = (props: Props) => {
  ...
  return (
    <Select
      ...
      disabled={props.currentMode !== FACE_LANDMARK_DETECTION_MODE.toString()}
      ...
      <SelectContent className="text-start">
        {modeList && modeList.length > 0 ? (
          modeList.map((mode, index) => {
            return (
              <SelectItem key={index} value={mode.mode.toString()}>
                {mode.name}
              </SelectItem>
            );
          })
        ) : (
          <SelectItem value={props.currentMode}>Models not loaded</SelectItem>
        )}
      </SelectContent>
    </Select>
  );
