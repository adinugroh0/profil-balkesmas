...
const FaceLandmarkDetection = (() => {
  ...
  const initModel = async (vision: any): Promise<ModelLoadResult> => {
    ...
    try {
      if (vision) {
        ...
        faceLandmark = await FaceLandmarker.createFromOptions(vision, config);
  ...
  const getMinFacePresenceConfidence = (): number => minFacePresenceConfidence;
  ...
  const detectFace = (video: HTMLVideoElement): FaceLandmarkerResult | null => {
    if (faceLandmark) {
      try {
        const detection: FaceLandmarkerResult = faceLandmark.detectForVideo(
          video,
          performance.now()
        );
    ...
    if (landmarks) {
      ...
      landmarks.forEach((landmark: NormalizedLandmark[]) => {
        landmark.forEach((data: NormalizedLandmark) => {
          ...
          const circleGeo = new CircleGeometry(CIRCLE_RADIUS, CIRCLE_SEGMENTS);
