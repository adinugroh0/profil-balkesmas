import Drawing3d from "@/src/lib/Drawing3d";
import {
  DELEGATE_GPU,
  DelegateMode,
...
const GestureRecognition = (() => {
  ...
  const getMinHandPresenceConfidence = (): number => minHandPresenceConfidence;
  ...
  const draw = (
    ...
    if (results) {
      ...
      results.handedness.forEach((hand: Category[], index: number) => {
        ...
        if (landmark) {
          Object.entries(fingerJoints).forEach((entry: [string, number[]]) => {
            ...
            const material = new LineMaterial({
              color: hand[0].categoryName === "Left" ? "#FF0F0F" : "#00B612",
          ...
          landmark.forEach((data: NormalizedLandmark, idx: number) => {
            ...
            const ringMat = new THREE.MeshBasicMaterial({
              depthTest: true,
              depthWrite: true,
              color: hand[0].categoryName === "Left" ? "#00B612" : "#FF0F0F",
              side: THREE.DoubleSide,
            });
  ...
  return {
    ...
    CONFIG_GESTURE_MIN_DETECTION_CONFIDENCE: CONFIG_MIN_DETECTION_CONFIDENCE,
    CONFIG_GESTURE_MAX_DETECTION_CONFIDENCE: CONFIG_MAX_DETECTION_CONFIDENCE,
    ...
    CONFIG_GESTURE_DEFAULT_HAND_SLIDER_STEP: CONFIG_DEFAULT_HAND_SLIDER_STEP,
